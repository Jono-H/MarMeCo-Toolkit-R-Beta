# Tracking data: Format and merge CSV files
...
```{r track-yelk-setup, include=FALSE}
...
```{r track-yelk-load-packages, include = TRUE}
...
```{r tracking-format-merge, include=TRUE}

stdb.df <- read_xlsx("./data-testing/tracking-data-input-stdb/Template_Datapoints.xlsx",
                     sheet = "Template")

head(data.frame(stdb.df))
...
```{r track-yelk-input-parameters, include = TRUE}
...
```{r track-yelk-load-data, include = TRUE}
...
```{r track-yelk-sbtd-format, include=TRUE}
...
For the following columns, you may notice a few things: <br>
...
```{r track-yelk-explore-data, include=TRUE}
...
```{r track-yelk-ind-formatFields, include = TRUE}
...
```{r track-yelk-dataGroup, include=TRUE}
...
```{r track-yelk-remove-duplicates, include=TRUE}
...
```{r track-yelk-visualise-data, include=TRUE}
...
```{r track-yelk-ind-create, include=TRUE}
...
```{r track-yelk-all-shapefile, include=TRUE, eval = FALSE}
...
```{r track-yelk-all-plots, include=TRUE, eval = FALSE}
...
```{r track-yelk-beg-end-tracks, include=TRUE}
...
```{r track-yelk-individual-plots, include = TRUE, eval = FALSE}
...
```{r track-yelk-individual-shapefile, include = TRUE, eval = FALSE}
...
```{r track-yelk-speed-all, include = TRUE}
...
```{r track-yelk-ind-col-locs, include = TRUE}
...
```{r track-yelk-ind-col-locs-checking, include = TRUE}
...
**When not to apply tripSplit():** "If your data does not relate to a central place forager (CPF), OR a time when an animal may be exhibiting central place foraging behaviours, then skip this section and those relating to CPF data."
...
```{r track-yelk-ind-tripSplit, include = TRUE}
...
```{r track-yelk-ind-tripSplit-review, include = TRUE}
...
```{r track-yelk-ind-mapTrips, include = TRUE}
...
```{r track-yelk-ind-keep-complete-trips-choose, include = TRUE}
...
```{r track-yelk-five-points, include = TRUE}
...
```{r track-yelk-tripSummary, include=TRUE}
...
```{r yelk-data-summary-tripSummary, include = TRUE}
...
To implement track2KBA fully, you need data approximating an even sampling interval i.e. location points must be regularly spaced in time. Determine how "gappy" the tracking data is (time intervals between location data) This is an important step for almost all tracking data analyses. If your data is not filtered / cleaned correctly, results may be spurious.

```{r track-yelk-ind-sampling-interval, include = TRUE}
...
```{r track-yelk-ind-sampling-interval-review, include = TRUE}
...
If you have trips where the maximum time gap in seconds (`max_timegap_secs`) is extremely different to the *median of median time gaps* in minutes, then it's likely you need to consider why this is the case for those trips. - Did you specify tripSplit parameters incorrectly? - Does the data require further cleaning in some other way? - What else to consider
...
["CONSIDER: What is the best way to provide advice on appropriate time gap selection for interpolation based on results above vs. comparison with intended sampling interval."]
...
```{r track-yelk-ind-buffer-points, include = TRUE}
...
```{r track-yelk-metadata-save, include=TRUE}
...
```{r track-yelk-interpolation-linear-all, include = TRUE}
...
```{r track-yelk-interpolation-all-review, include = TRUE}
...
```{r tracks-yelk-meta-bind, include=TRUE}
...
```{r track-yelk-projecTracks, include=TRUE}
...
```{r track-yelk-findScale, include=TRUE}
...
```{r track-yelk-estSpaceUse, include=TRUE}
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## estSpaceUse() ----
...
```{r track-yelk-review-smoothing, include=TRUE}
...
```{r track-yelk-repAssess, include = TRUE}
...
```{r yelk-data-iba-polygons, include=TRUE}
