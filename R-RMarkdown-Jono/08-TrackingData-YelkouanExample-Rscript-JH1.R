...
#df_sbtd$date_gmt <- as.POSIXct(df_sbtd$date_gmt, format = "%d/%m/%Y", tz = "gmt")
...
for(i in 1:length(unique(dataGroup$ID))){
  ...
  print(paste0(i,": ", nrow(subset(trip_obj, trip_obj$Filter == F)),
               " points removed for bird ID: ",temp$ID[1]))
...
##
unique(tracks_speed$ID)

## specify individual
p = "20_Tag40118_Z-175"

## Plot the speed filtered vs. original tracks
map <- leaflet() %>% ## start leaflet plot
  addProviderTiles(providers$Esri.WorldImagery, group = "World Imagery") %>% 
  ## points before speed filter
  addCircleMarkers(data = subset(dataGroup, dataGroup$ID == p),
                   label = dataGroup$DateTime , radius = 3,
                   fillColor = "cyan",
                   fillOpacity = 0.5, stroke = F) %>% 
  ## points after speed filter
  addCircleMarkers(data = subset(tracks_speed, tracks_speed$ID == p),
                   label = tracks_speed$DateTime , radius = 3,
                   fillColor = "red",
                   fillOpacity = 0.5, stroke = F) 
